# This code is taken from "https://github.com/IDEA-FinAI/RagVL/blob/main/utils/metrics.py".
import re, string
import spacy
...
def longest_common_substring_length(seq1, seq2):
    ...
    # 두 시퀀스(seq1, seq2) 사이의 가장 긴 연속 공통 부분 문자열의 길이를 반환.
    
    m, n = len(seq1), len(seq2)
    # dp[i+1][j+1]는 seq1[i]와 seq2[j]가 일치할 경우 
    # 그 이전 dp[i][j] 값에 1을 더한 값.
...
def compute_score(gold, pred):
    """
    gold와 pred 리스트에서, gold와 pred의 가장 긴 연속 공통 부분 문자열의 길이를 
    gold의 길이로 나눈 점수를 반환합니다.
    """
    if not gold:
        return 0  # gold가 빈 경우 예외 처리
    lcs_length = longest_common_substring_length(gold, pred)
    score = lcs_length / len(gold)
    return score
